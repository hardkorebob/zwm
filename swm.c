...
px (const char *hex)
{
  ...
  c.green = (unsigned short)((hex_val (hex[3]) * 16 + hex_val (hex[4])) * 256);
  c.blue  = (unsigned short)((hex_val (hex[5]) * 16 + hex_val (hex[6])) * 256);
...
should_float (Window wid)
{
  ...
  unsigned long ni, after;
  unsigned char *data = NULL;
  if (XGetWindowProperty (dpy, wid, a_wm_type, 0, 32, False, XA_ATOM, 
                          &type, &fmt, &ni, &after, &data)
          == Success
      && data && ni > 0)
    {
